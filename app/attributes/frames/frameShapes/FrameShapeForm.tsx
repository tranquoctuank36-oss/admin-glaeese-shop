...
export type FrameShapeFormValues = {
  name: string;
  slug: string;
  isActive: boolean;
};

export default function FrameShapeForm({
  initial,
  onSubmit,
  onCancel,
  submitLabel = "Save",
}: {
  initial: FrameShapeFormValues;
  onSubmit: (values: FrameShapeFormValues) => Promise<void> | void;
  onCancel: () => void;
  submitLabel?: string;
}) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(detail || "An error occurred while saving.");
    } finally {
      setLoading(false);
    }
  };
