...
export default function DiscountTargetSelector({
  ...
  const loadProducts = async () => {
    try {
      ...
      // Fetch full product details to get complete variant info (colors, etc.)
      const productsWithFullDetails = await Promise.all(
        result.data.map(async (product) => {
          try {
            ...
            return product; // Fallback to basic info
          }
        })
      );

      setProducts(productsWithFullDetails);
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="flex flex-col w-full max-w-[calc(100%-2rem)] sm:max-w-[600px] max-h-[90vh]">
          <DialogHeader>
            <DialogTitle>Thêm biến thể sản phẩm vào chương trình giảm giá</DialogTitle>
          </DialogHeader>
          ...
          {selectedVariants.size > 0 && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-2 flex items-center justify-between">
              <span className="text-sm text-blue-700 font-medium">
                {selectedVariants.size} biến thể đã chọn
              </span>
              <Button
                type="button"
                size="sm"
